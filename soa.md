...
由于后两个方法依赖于doctors的返回，所以需要等待doctors的返回，然后再并发执行：
...
当然，由于采用了异步的方法，在发起调用后无法立即得到返回值，于此同时，我们可以继续组装、计算其它结果集，待mq返回填充后，再继续。这样无论从设计到实施，都要比上边的顺序执行要复杂，对程序员的编码能力也会有更大的挑战。此时，就需要我们在复杂度和执行效率上做平衡了。由于SOA的接口都是无状态的，即相互依赖是有限的，大部分都是查询类接口，所以我还是倾向于在开发的时候认真思考归类，然后并发调用。

多说一句，这种思路目前被很多前端框架大量使用，即利用浏览器并发调用后台接口，然后各自拼装数据，互不干涉。
